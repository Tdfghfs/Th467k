...
export async function createConversation(
  ...
  const personalityNames: Record<string, string> = {
    programmer: "محادثة البرمجة"
  };

  const result = await db.insert(conversations).values({
    userId,
    personality,
    title: personalityNames[personality] || "محادثة جديدة",
  });
...
export async function addMessage(
  ...
  const created = await db.select().from(messages).where(eq(messages.id, result[0].insertId)).limit(1);
  return created[0];
}
